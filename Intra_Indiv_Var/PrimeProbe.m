...
load('subtrials.mat'); %Get trial orders
%--------------------------------------------------------------------------
%                        Participant Setup
%--------------------------------------------------------------------------
...
for orderchecker=1:size(subtrials,2);
    if strcmp(subID,subtrials{2,orderchecker});
       stimOrder=subtrials{1,orderchecker};
    end
end
...
DrawFormattedText(window, 'Ignore the first letter and respond to the second letter: A = hit "c" with right index, B = hit "v" with right middle, Y = hit "b" with right ring, Z = hit "n" with right pinky', 'center', yCenter, white);
