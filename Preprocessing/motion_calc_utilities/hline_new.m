function hhh=hline_new(y,in1,in2,in3)
...
% and 'label', which applies a text label to the graph near the line.  The
...
% 8 November 2001
% Modified Timothy Laumann 03/02/11
...
if length(y)>1  % vector input
    for I=1:length(y)
        ...
        h(I)=hline_new(y(I),linetype, linewidth, label);
    end
else
    switch nargin
    case 1
    ...
    x=get(gca,'xlim');
    h=plot(x,[y y],linetype,'LineWidth',linewidth);
    if ~isempty(label)
        yy=get(gca,'ylim');
        yrange=yy(2)-yy(1);
        yunit=(y-yy(1))/yrange;
