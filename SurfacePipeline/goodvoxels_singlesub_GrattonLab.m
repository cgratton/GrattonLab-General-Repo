function  goodvoxels_singlesub_GrattonLab(funcvol, surfdir, outfile,subject,sequence_name)
% originally from EMN
...
system(['csh create_ribbon_singlesub_GrattonLab.csh ' subject ' ' surfdir ' T1_biascorr_MNI.nii.gz 3']);

meanname = [outfile '_' sequence_name '_mean'];
stdname = [outfile '_' sequence_name '_sd1'];

system(['module load fsl/5.0.8; fslmaths ' funcvol ' -Tmean ' meanname]);
system(['module load fsl/5.0.8; fslmaths ' funcvol ' -Tstd ' stdname]);

covname = [outfile '_' sequence_name '_cov'];
...
system(['module load fsl/5.0.8; fslmaths ' covname '_ribbon_norm -bin -s ' neighsmooth ' ' outfile '_' sequence_name '_SmoothNorm']);
system(['module load fsl/5.0.8; fslmaths ' covname '_ribbon_norm -s ' neighsmooth ' -div ' outfile '_' sequence_name '_SmoothNorm -dilD ' covname '_ribbon_norm_s' neighsmooth]);
...
system(['module load fsl/5.0.8; fslmaths ' meanname ' -bin ' outfile '_' sequence_name '_mask']);
system(['module load fsl/5.0.8; fslmaths ' covname '_norm_modulate -thr ' num2str(Upper) ' -bin -sub ' outfile '_' sequence_name '_mask -mul -1 ' outfile '/' sequence_name '_goodvoxels']);
...
delete([outfile '_' sequence_name '_SmoothNorm.nii.gz'])
...
delete([outfile '_' sequence_name '_mask.nii.gz']);
